...
Board::Board(LawnApp* theApp)
{
	...
	mZombies.DataArrayInitialize(16384U, "zombies");
	mPlants.DataArrayInitialize(8192U, "plants");
	mProjectiles.DataArrayInitialize(16384U, "projectiles");
	mCoins.DataArrayInitialize(8192U, "coins");
	mLawnMowers.DataArrayInitialize(64U, "lawnmowers");
	mGridItems.DataArrayInitialize(2048U, "griditems");
	...
	for (int i = 0; i < MAX_GRID_SIZE_X; i++)
	{
		for (int j = 0; j < MAX_GRID_SIZE_Y; j++)
		{
			...
			// Set Y offset to 0 for the top (j==0) and bottom lane (j==MAX_GRID_SIZE_Y-1)
			if (j == 0 || j == MAX_GRID_SIZE_Y - 1)
				mGridCelOffset[i][j][1] = 0;
			else
...
void Board::PickZombieWaves()
{
	...
	for (int aWave = 0; aWave < mNumWaves; aWave++)
	{
		...
		// Tier 1: Waves 1-3 (Preparation Phase)
		if (aWave < 3)
		{
			if (mApp->IsAdventureMode() && mApp->HasFinishedAdventure() && mLevel != 5)
				aZombiePoints = 2;
			else
				aZombiePoints = 1;
		}
		// Tier 2: Waves 4-6 (2x Difficulty)
		else if (aWave < 6)
		{
			if (mApp->IsAdventureMode() && mApp->HasFinishedAdventure() && mLevel != 5)
				aZombiePoints = aWave + 2;
			else
		...
		{
			if (mApp->IsAdventureMode() && mApp->HasFinishedAdventure() && mLevel != 5)
				aZombiePoints = aWave + 3; // 3x multiplier for New Game+
			else
				aZombiePoints = aWave + 2; // 3x multiplier (2.0) for the first playthrough
		}
		// Tier 4: Waves 11-14 and beyond (4x Difficulty)
		else if (aWave < 14)
		{
			if (mApp->IsAdventureMode() && mApp->HasFinishedAdventure() && mLevel != 5)
				aZombiePoints = aWave + 4; // 4x multiplier for New Game+
			else
				aZombiePoints = aWave + 3; // 3x multiplier (3.0) for the first playthrough
		}
		else
		{
			if (mApp->IsAdventureMode() && mApp->HasFinishedAdventure() && mLevel != 5)
				aZombiePoints = aWave + 5; // 5x multiplier for New Game+
			else
...
void Board::PickBackground()
{
	...
	if (StageHasGraveStones())
	{
		...
		else if (mBackground == BackgroundType::BACKGROUND_7_EVENING)
		{
			...
			else if (mLevel == 61 || mLevel == 62 || mLevel == 63)
			{
				AddGraveStones(6, 1, aLevelRNG);
				AddGraveStones(7, 1, aLevelRNG);
				AddGraveStones(8, 2, aLevelRNG);
			}
			else if (mLevel == 64 || mLevel == 65 || mLevel == 66)
			{
				AddGraveStones(5, 1, aLevelRNG);
				AddGraveStones(6, 1, aLevelRNG);
				AddGraveStones(7, 2, aLevelRNG);
			...
			else if (mLevel == 67 || mLevel == 68 || mLevel == 69)
			{
				AddGraveStones(4, 1, aLevelRNG);
				AddGraveStones(5, 2, aLevelRNG);
				AddGraveStones(6, 2, aLevelRNG);
...
Rect Board::GetFertilizerButtonRect()
{
	Rect aShovelRect = GetShovelButtonRect();
	// This places the fertilizer button immediately to the right of the shovel
	return Rect(aShovelRect.mX + aShovelRect.mWidth, aShovelRect.mY, IMAGE_SHOVELBANK->GetWidth(), IMAGE_SHOVELBANK->GetHeight());
}
...
void Board::InitLevel()
{
	...
	mNumSunsFallen = 0;
	if (!StageIsNight() && mBackground != BackgroundType::BACKGROUND_7_EVENING)
	{
...
void Board::StartLevel()
{
	...
	for (int i = 0; i < mSeedBank->mNumPackets; i++)
	{
		if (mSeedBank->mSeedPackets[i].mPacketType != SEED_IMITATER)
		{
			mApp->mLastPlantPlanted = mSeedBank->mSeedPackets[i].mPacketType;
			break;
		}
	}
}
...
void Board::FadeOutLevel()
{
	...
	{
		mApp->mMusic->StopAllMusic();
		if (mApp->IsAdventureMode() && mLevel == 60)
		{
...
Plant* Board::AddPlant(int theGridX, int theGridY, SeedType theSeedType, SeedType theImitaterType)
{
	...
	{
		Plant* aLilyPad = GetTopPlantAt(theGridX, theGridY, TOPPLANT_ONLY_UNDER_PLANT);
		Plant* aFlowerPot = GetFlowerPotAt(theGridX, theGridY);
		if (aLilyPad && aLilyPad->mSeedType == SEED_LILYPAD && aFlowerPot)
		{
			aPlant->mIsOnPotAndLily = true;
		}
	}
	...
	if (theSeedType == SeedType::SEED_PEASHOOTER ||
		...
		theSeedType == SeedType::SEED_COMMANDOPEA ||
	...
	Plant* aPumpkin = GetPumpkinAt(theGridX, theGridY);
	Plant* aPlantInPot = GetTopPlantAt(theGridX, theGridY, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);

	// Apply buff if a plant is inside a pumpkin
	if (aPumpkin != nullptr && aPlantInPot != nullptr && aPlantInPot != aPumpkin && theSeedType != SeedType::SEED_INSTANT_COFFEE)
	{
		// Only store the original health if it hasn't been stored already
		if (aPlantInPot->mPlantMaxHealthOriginal == 0)
		{
			aPlantInPot->mPlantMaxHealthOriginal = aPlantInPot->mPlantMaxHealth;
		}

		// Double the plant's max health and heal it to full
		aPlantInPot->mPlantMaxHealth *= 2;
		aPlantInPot->mPlantHealth = aPlantInPot->mPlantMaxHealth;
	}
...
bool Board::IsValidCobCannonSpotHelper(int theGridX, int theGridY)
{
	...
	if (aPlantOnLawn.mNormalPlant == nullptr && CanPlantAt(theGridX, theGridY, SeedType::SEED_PEASHOOTER) == PlantingReason::PLANTING_OK)
		return true;
...
{
	if (mProjectiles.mSize >= mProjectiles.mMaxSize - 1)
	{
		return nullptr;
	}
...
ZombieType Board::PickGraveRisingZombieType(int theZombiePoints)
{
	ZombieType aPossibleZombies[] = {
		...
		ZombieType::ZOMBIE_DOOR_CONEHEAD,
		ZombieType::ZOMBIE_DOOR_BUCKETHEAD
	};

	int aNumPossibleZombies = sizeof(aPossibleZombies) / sizeof(aPossibleZombies[0]);
	TodWeightedArray* aZombieWeightArray = new TodWeightedArray[aNumPossibleZombies];
	int aWeightCount = 0;

	for (int i = 0; i < aNumPossibleZombies; i++)
	{
		ZombieType aZombieType = aPossibleZombies[i];
		const ZombieDefinition& aZombieDef = GetZombieDefinition(aZombieType);

		// Check if the zombie is allowed to spawn on the current level
		if (CanZombieSpawnOnLevel(aZombieType, mLevel) && theZombiePoints >= aZombieDef.mZombieValue)
		{
			aZombieWeightArray[aWeightCount].mItem = (int)aZombieType;
			aZombieWeightArray[aWeightCount].mWeight = aZombieDef.mPickWeight;
			aWeightCount++;
		}
	}

	// If no zombies from our list are allowed, default to a normal zombie.
	ZombieType result;
	if (aWeightCount == 0)
	{
		result = ZombieType::ZOMBIE_NORMAL;
	}
	else
	{
		result = (ZombieType)TodPickFromWeightedArray(aZombieWeightArray, aWeightCount);
	}

	delete[] aZombieWeightArray; // Add this line to free the dynamically allocated memory
	return result;
}
...
bool Board::RowCanHaveZombieType(int theRow, ZombieType theZombieType)
{
	...
	else if (IsZombieTypePoolOnly(theZombieType) && mPlantRow[theRow] != PLANTROW_POOL)
	{
		return false;
	}
...
int Board::PickRowForNewZombie(ZombieType theZombieType)
{
	...
	for (int aRow = 0; aRow < MAX_GRID_SIZE_Y; aRow++)
	{
		...
		else
		{
			...
			bool isPoolRow = (mPlantRow[aRow] == PlantRowType::PLANTROW_POOL);

			if (aWavesMowered <= 1)
			{
				mRowPickingArray[aRow].mWeight = isPoolRow ? 0.02f : 0.01f;
			}
			else if (aWavesMowered <= 2)
			{
				mRowPickingArray[aRow].mWeight = isPoolRow ? 1.0f : 0.5f;
			}
			else
			{
				mRowPickingArray[aRow].mWeight = isPoolRow ? 2.0f : 1.0f;
			}
		}
	}
	// Check if all rows ended up with a weight of 0
	bool allWeightsZero = true;
	for (int i = 0; i < MAX_GRID_SIZE_Y; ++i)
	{
		if (mRowPickingArray[i].mWeight > 0)
		{
	...
	// If all weights are zero, reset the weights of all VALID rows to 1
	if (allWeightsZero)
	{
...
{
	int aRow = PickRowForNewZombie(theZombieType);
	if (aRow == -1)
	{
		return nullptr; // Abort the spawn if no valid row could be found
	}
...
PlantingReason Board::CanPlantAt(int theGridX, int theGridY, SeedType theSeedType)
{
	...
	{
		Plant* aTargetPlant = aPlantOnLawn.mNormalPlant;
		if (aHasGrave || (aTargetPlant && aTargetPlant->mSeedType != SEED_PUMPKINSHELL && aTargetPlant->mSeedType != SEED_GRAVEBUSTER))
		{
			return PlantingReason::PLANTING_OK;
		}
	...
	if (theSeedType == SeedType::SEED_INSTANT_COFFEE)
	{
		...
		if (!aPlantOnLawn.mNormalPlant || aPlantOnLawn.mNormalPlant->mWakeUpCounter > 0 ||
	...
	if (theSeedType == SeedType::SEED_LILYPAD || theSeedType == SeedType::SEED_TANGLEKELP || theSeedType == SeedType::SEED_SEASHROOM)
{
    ...
    else if (theSeedType == SeedType::SEED_LILYPAD && aHasFlowerPot)
    {
        return aNormalPlant ? PlantingReason::PLANTING_NOT_HERE : PlantingReason::PLANTING_OK;
    }
    else
    {
	...
	Plant* aFlowerPotOnTile = GetFlowerPotAt(theGridX, theGridY);
	if (aHasLilypad && aFlowerPotOnTile && !aNormalPlant)
	{
		/* unused
	...
	if (aUnderPlant)
	{
		if (theSeedType == SeedType::SEED_CATTAIL)
		{
			...
			if (aUnderPlant->mOnBungeeState != PlantOnBungeeState::GETTING_GRABBED_BY_BUNGEE)
			{
				return PlantingReason::PLANTING_OK;
			}
			/* unused
	...
	if (aNormalPlant)
	{
		if (theSeedType == SEED_SUNSHROOM && aNormalPlant->mSeedType == SEED_SUNSHROOM)
		{
			if (aNormalPlant->mState == STATE_SUNSHROOM_SMALL)
			{
				return PlantingReason::PLANTING_OK;
			}
			if (aNormalPlant->mState == STATE_SUNSHROOM_BIG)
			{
				if (aNormalPlant->mSunshroomSteaks < 4)
				{
					return PlantingReason::PLANTING_OK;
				}
				else
				{
					return PlantingReason::PLANTING_SUNSHROOM_LIMIT_REACHED;
				}
			}
		}
...
void Board::MouseDownWithPlant(int x, int y, int theClickCount)
{
	...
	if (aPlantingSeedType == SEED_SUNSHROOM)
	{
		Plant* aExistingPlant = GetTopPlantAt(aGridX, aGridY, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
		if (aExistingPlant && aExistingPlant->mSeedType == SEED_SUNSHROOM)
		{
			// Case 1: Grow a small Sun-shroom to a big one.
			if (aExistingPlant->mState == STATE_SUNSHROOM_SMALL)
			{
				aExistingPlant->PlayBodyReanim("anim_grow", REANIM_PLAY_ONCE_AND_HOLD, 10, 12.0f);
				aExistingPlant->mState = STATE_SUNSHROOM_GROWING;
				mApp->PlayFoley(FOLEY_PLANTGROW);
			}
			// Case 2: Upgrade a big Sun-shroom's production rate.
			else if (aExistingPlant->mState == STATE_SUNSHROOM_BIG && aExistingPlant->mSunshroomSteaks < 4)
			{
				aExistingPlant->mLaunchRate -= 250;
				aExistingPlant->mSunshroomSteaks++;
				mApp->PlayFoley(FOLEY_PLANTGROW);
				mApp->AddTodParticle(aExistingPlant->mX + 40, aExistingPlant->mY + 40, aExistingPlant->mRenderOrder + 1, PARTICLE_STARBURST)->OverrideColor(nullptr, Color(255, 255, 100));
			}
			// Case 3: The Sun-shroom is maxed out or busy growing. Show message and stop.
			else
			{
				mApp->PlaySample(SOUND_BUZZER);
				DisplayAdvice(_S("It's Already at its Limit!"), MessageStyle::MESSAGE_STYLE_HINT_FAST, AdviceType::ADVICE_NONE);
				return; // VERY IMPORTANT: Stop without planting or consuming sun.
			}

			// If an upgrade happened, consume the seed/sun and stop the function here.
			TakeSunMoney(GetCurrentPlantCost(aPlantingSeedType, SEED_NONE));
	...
	if (aReason != PlantingReason::PLANTING_OK)
	{
		...
		else if (aReason == PlantingReason::PLANTING_SUNSHROOM_LIMIT_REACHED)
		{
			DisplayAdvice(_S("It's Already at its Limit!"), MessageStyle::MESSAGE_STYLE_HINT_FAST, AdviceType::ADVICE_NONE);
		}
	...
	if ((aPlantingSeedType == SeedType::SEED_WALLNUT || aPlantingSeedType == SeedType::SEED_TALLNUT ||
		aPlantingSeedType == SeedType::SEED_EXPLODE_O_NUT || aPlantingSeedType == SeedType::SEED_DOOM_NUT) && aNormalPlant)
	{
...
void Board::MouseDownWithTool(int x, int y, int theClickCount, CursorType theCursorType)
{
	...
	else if (theCursorType == CURSOR_TYPE_FERTILIZER)
	{
		Plant* aPlant = ToolHitTest(x, y);
		if (aPlant != nullptr)
		{
			// Check if the plant is actually upgradable
			if (aPlant->mSeedType == SEED_KERNELPULT)
			{
				// Check for a second Kernel-pult in the next column over
				Plant* aSecondPlant = GetTopPlantAt(aPlant->mPlantCol + 1, aPlant->mRow, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
				if (aSecondPlant != nullptr && aSecondPlant->mSeedType == SEED_KERNELPULT)
				{
					...
					aZenTool->mGridItemType = GRIDITEM_ZEN_TOOL;
					aZenTool->mGridItemState = GRIDITEM_STATE_ZEN_TOOL_FERTILIZER;
					aZenTool->mGridX = aPlant->mPlantCol;
					aZenTool->mGridY = aPlant->mRow;
					aZenTool->mRenderOrder = aPlant->mRenderOrder + 1;

					Reanimation* aReanim = mApp->AddReanimation(aPlant->mX + 5, aPlant->mY - 30, aZenTool->mRenderOrder, REANIM_ZENGARDEN_FERTILIZER);
					aReanim->mLoopType = REANIM_PLAY_ONCE_AND_HOLD;
					aZenTool->mGridItemReanimID = mApp->ReanimationGetID(aReanim);
				}
				else
				{
					// The second plant is missing, show error message
					mApp->PlaySample(SOUND_BUZZER);
					DisplayAdvice(_S("You need 2 Kernel-pults to make a Cob Cannon."), MESSAGE_STYLE_HINT_FAST, ADVICE_NONE);
				}
			}
			...
			{
				// Plant is not upgradable, show a message
				mApp->PlaySample(SOUND_BUZZER);
				DisplayAdvice(_S("This plant cannot be upgraded."), MESSAGE_STYLE_HINT_FAST, ADVICE_NONE);
			}
		}
		else
		{
...
bool Board::MouseHitTest(int x, int y, HitResult* theHitResult)
{
	...
	if (mShowShovel && mApp->CanShowZenGarden() && mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_FERTILIZER] > PURCHASE_COUNT_OFFSET && CanInteractWithBoardButtons())
	{
		...
		{
			theHitResult->mObjectType = GameObjectType::OBJECT_TYPE_FERTILIZER;
			return true;
		}
	}
...
void Board::MouseUp(int x, int y, int theClickCount)
{
	...
	if (CanInteractWithBoardButtons() && theClickCount > 0)
	{
		...
		if (mFastButton->IsMouseOver() && !mApp->GetDialog(Dialogs::DIALOG_GAME_OVER) && !mApp->GetDialog(Dialogs::DIALOG_LEVEL_COMPLETE) && mBoardFadeOutCounter < 0)
		{
			...
			UpdateCursor();
			if (mApp->isFastMode == 0)
			{
				mApp->isFastMode = 1;
			}
			else if (mApp->isFastMode == 1 || mApp->isFastMode == 2)
			{
				mApp->isFastMode = 0;
				gSlowMo = false;
			}
...
void Board::SpawnZombieWave()
{
	...
	{
		if (!(mCurrentWave >= 0 && mCurrentWave < MAX_ZOMBIE_WAVES && mCurrentWave < mNumWaves)) {
			TodTraceAndLog("Assertion failed: mCurrentWave=%d, MAX_ZOMBIE_WAVES=%d, mNumWaves=%d",
				mCurrentWave, MAX_ZOMBIE_WAVES, mNumWaves);
			TOD_ASSERT(false);
		}
		for (int i = 0; i < MAX_ZOMBIES_IN_WAVE; i++)
		{
	...
	{
		// If it's a night level, start the grave spawn countdown on every flag wave.
		if (StageHasGraveStones())
		{
		...
		// Otherwise (for non-night levels), keep the original behavior.
		else if (mCurrentWave == mNumWaves - 1)
		{
			mRiseFromGraveCounter = 210;
		}
	}
...
void Board::Update()
{
	...
	if (mPaused && (mApp->isFastMode == 1 || mApp->isFastMode == 2))
	{
		mApp->isFastMode = 0;
		gSlowMo = false;
		gFastMo = false;
	...
	if (mFastButton != nullptr && !mFastButton->mBtnNoDraw)
	{
		...
		else if (mApp->isFastMode == 1)
        {
            mFastButton->mButtonImage = IMAGE_FASTBUTTON_HIGHLIGHT;
            mFastButton->mOverImage = IMAGE_FASTBUTTON_HIGHLIGHT;
            mFastButton->mDownImage = IMAGE_FASTBUTTON;
        }
		else if (mApp->isFastMode == 2)
		{
			mFastButton->mButtonImage = IMAGE_SLOWMO_HIGHLIGHT;
			mFastButton->mOverImage = IMAGE_SLOWMO_HIGHLIGHT;
			mFastButton->mDownImage = IMAGE_FASTBUTTON;
		}
	}
	...
	if (mApp->mLastPlantPlanted != SEED_NONE)
	{
		for (int i = 0; i < mSeedBank->mNumPackets; i++)
		{
			SeedPacket& aPacket = mSeedBank->mSeedPackets[i];
			if (aPacket.mPacketType == SEED_IMITATER)
			{
				// If the copied type is different, update it
				if (aPacket.mImitaterType != mApp->mLastPlantPlanted)
				{
					aPacket.mImitaterType = mApp->mLastPlantPlanted;
				}
			}
		}
	}
...
void Board::DrawBackdrop(Graphics* g)
{
	...
	case BackgroundType::BACKGROUND_7_EVENING:			aBgImage = Sexy::IMAGE_BACKGROUND7;						break;
...
{
	if (theCurRenderItem >= MAX_RENDER_ITEMS) return;
...
void Board::DrawGameObjects(Graphics* g)
{
...

	{
		...
		while (IteratePlants(aPlant))
		{
			if (aPlant->mOnBungeeState == PlantOnBungeeState::NOT_ON_BUNGEE)
			{
				...
				{
					if (aRenderItemCount >= MAX_RENDER_ITEMS) break;
	...
	if (mApp->IsStormyNightLevel() || mApp->mGameMode == GameMode::GAMEMODE_CHALLENGE_RAINING_SEEDS || IsNightRoof())
	{
	...
	for (int i = 0; i < aRenderItemCount; i++)
	{
		...
		case RenderObjectType::RENDER_ITEM_HEALTHBAR_PLANT:
		{
			...
			if (isPumpkin)
			{
				Plant* aInnerPlant = GetTopPlantAt(aPlant->mPlantCol, aPlant->mRow, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
				if (aInnerPlant != nullptr && aInnerPlant != aPlant)
				{
					break; // Skip standalone Pumpkin bar when inner plant already draws it
				}
			}
			Color baseColor;
			...
			Color textColor = Color::White;
			bool drawBarOutline = true;
			int currentOffsetY = baseBarOffsetY;
			int extraOffsetX = ((aPlant->mSeedType != SeedType::SEED_IMITATER && isPumpkin) || aPlant->mSeedType == SeedType::SEED_TALLNUT) ? 10 : 0;
			int barSpacing = barHeight + textOffsetY + 16;

			// Layer 1 (Bottom): Main Plant HP Bar
			if (aPlant->mPlantHealth > 0)
			{
				DrawHealthbar(g, rect, maxColor, aPlant->mPlantMaxHealth, baseColor, aPlant->mPlantHealth, barWidth, barHeight, extraOffsetX, currentOffsetY, textColor, FONT_BRIANNETOD12, textOffsetY, Color::Black, textOutlineOffset, drawBarOutline);
				currentOffsetY += barSpacing;
			}

			// Layer 2 (Middle - if Pumpkin present): Pumpkin Armor Bar
			Plant* aPumpkinOnTile = GetPumpkinAt(aPlant->mPlantCol, aPlant->mRow);
			if (aPumpkinOnTile != nullptr && aPumpkinOnTile != aPlant && aPumpkinOnTile->mPlantHealth > 0)
			{
				Color pumpkinBaseColor = Color(255, 165, 0);
				Color pumpkinMaxColor = Color(150, 80, 0);
				DrawHealthbar(g, rect, pumpkinMaxColor, aPumpkinOnTile->mPlantMaxHealth, pumpkinBaseColor, aPumpkinOnTile->mPlantHealth, barWidth, barHeight, extraOffsetX, currentOffsetY, textColor, FONT_BRIANNETOD12, textOffsetY, Color::Black, textOutlineOffset, drawBarOutline);
				currentOffsetY += barSpacing;
			}

			// Layer 3 (Top): Shield Bar
			if (aPlant->mShieldHealth > 0)
			{
				Color shieldBaseColor = Color(0, 180, 255);
				Color shieldMaxColor = Color(0, 50, 150);
				DrawHealthbar(g, rect, shieldMaxColor, aPlant->mShieldMaxHealth, shieldBaseColor, aPlant->mShieldHealth, barWidth, barHeight, extraOffsetX, currentOffsetY, textColor, FONT_BRIANNETOD12, textOffsetY, Color::Black, textOutlineOffset, drawBarOutline);

				int barX = rect.mX + (rect.mWidth - barWidth) / 2 - extraOffsetX;
				int barY = rect.mY - barHeight - currentOffsetY;
				g->SetColor(Color(0, 40, 100, 230));
				g->FillRect(Rect(barX - 16, barY - 1, 13, 13));
				g->SetColor(Color(0, 200, 255, 255));
				g->DrawRect(Rect(barX - 16, barY - 1, 13, 13));
				g->SetColor(Color::White);
				g->FillRect(Rect(barX - 11, barY + 2, 3, 7));
				g->FillRect(Rect(barX - 13, barY + 4, 7, 3));
			}
			break;
		}
		default:
...
void Board::DrawHouseDoorBottom(Graphics* g)
{
	...
	case BackgroundType::BACKGROUND_7_EVENING:	g->DrawImage(Sexy::IMAGE_BACKGROUND7_GAMEOVER_INTERIOR_OVERLAY, -126, 225);		break;
	default:																													break;
	}
}
...
void Board::DrawHouseDoorTop(Graphics* g)
{
	...
	case BackgroundType::BACKGROUND_7_EVENING:	g->DrawImage(Sexy::IMAGE_BACKGROUND7_GAMEOVER_MASK, -130, 202);		break;
	default:																										break;
	}
}
...
void Board::DrawUIBottom(Graphics* g)
{
	...
	if (mShowShovel && mApp->CanShowZenGarden() && mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_FERTILIZER] >= PURCHASE_COUNT_OFFSET)
	{
		...
		if (mCursorObject->mCursorType != CURSOR_TYPE_FERTILIZER)
		{
			...
			g->DrawImage(IMAGE_FERTILIZER, aFertilizerRect.mX - 6, aFertilizerRect.mY - 7);
			if (aCharges == 0)
			{
				g->SetColorizeImages(false);
			}
...
void Board::UpdateFog()
{
	...
	while (IteratePlants(aPlant))
	{
		...
		else if (aPlant->mSeedType == SeedType::SEED_ICE_PLANTERN)
		{
			ClearFogAroundPlant(aPlant, 8);
		}
...
void Board::KeyChar(SexyChar theChar)
{
	...
	{
		if (mCursorObject->mCursorType != CursorType::CURSOR_TYPE_FERTILIZER)
		{
			if (mCursorObject->mCursorType == CursorType::CURSOR_TYPE_PLANT_FROM_BANK)
				RefreshSeedPacketFromCursor();
			PickUpTool(GameObjectType::OBJECT_TYPE_FERTILIZER);
		}
		else
		{
			ClearCursor();
	...
	if (mApp->IsAdventureMode())
	{
		if (theChar == _S('<'))
		{
			...
			{
				mLevel--;
				mApp->mPlayerInfo->mLevel = mLevel;
		...
		{
			mApp->mPlayerInfo->mLevel = 11;
			mApp->PreNewGame(GameMode::GAMEMODE_ADVENTURE, false);
		}
		else if (theChar == _S('#'))
		{
			mApp->mPlayerInfo->mLevel = 21;
			mApp->PreNewGame(GameMode::GAMEMODE_ADVENTURE, false);
		}
		else if (theChar == _S('$'))
		{
			mApp->mPlayerInfo->mLevel = 31;
			mApp->PreNewGame(GameMode::GAMEMODE_ADVENTURE, false);
		}
		else if (theChar == _S('%'))
		{
			mApp->mPlayerInfo->mLevel = 41;
			mApp->PreNewGame(GameMode::GAMEMODE_ADVENTURE, false);
		}
		else if (theChar == _S('^'))
		{
	...
	*/
	...
	if (theChar == _S('|'))
	{	
		mApp->mPlayerInfo->AddCoins(9999);
		mApp->PlaySample(SOUND_BUTTONCLICK);
		ShowCoinBank();
		return;
	}
	...
	if (theChar == 'F')
	{
		int dropX = mX + 400; // Adjust X position if necessary
		int dropY = mY + 300; // Adjust Y position if necessary

		AddCoin(dropX, dropY, CoinType::COIN_FERTILIZER, CoinMotion::COIN_MOTION_LAWNMOWER_COIN);

		// Optionally, play an appropriate sound effect.
		mApp->PlayFoley(FoleyType::FOLEY_FERTILIZER);
		return;
	}
...
bool Board::HasConveyorBeltSeedBank()
{
	return
		...
		(mApp->IsAdventureMode() && (mLevel == 50 || mLevel == 55)) ||
...
void Board::DropLootPiece(int thePosX, int thePosY, int theDropFactor)
{
	if (mApp->CanShowZenGarden() && mApp->mPlayerInfo->mPurchases[(int)StoreItem::STORE_ITEM_FERTILIZER] < PURCHASE_COUNT_OFFSET + 5)
	{
		if (Rand(100) < 5) // 5% chance
		{
			...
			AddCoin(thePosX - 40, thePosY, CoinType::COIN_FERTILIZER, CoinMotion::COIN_MOTION_COIN);
			return; // Exit the function so no other loot drops
		}
	}
...
void Board::UpdateGridItems()
{
	...
	while (IterateGridItems(aGridItem))
	{
		...
		{
			Reanimation* aReanim = mApp->ReanimationTryToGet(aGridItem->mGridItemReanimID);
			// Check if the animation has finished playing
			if (aReanim == nullptr || aReanim->mLoopCount > 0)
			{
				UpgradePlant(aGridItem->mGridX, aGridItem->mGridY);
				aGridItem->GridItemDie();
			}
		}

		if (aGridItem->mGridItemType == GRIDITEM_FIRE_PLACE)
		{
			...
			int damageThisTick = aGridItem->mSunCount > 0 ? aGridItem->mSunCount : 2;
			if (damageThisTick > 2 && (aGridItem->mGridItemCounter % 5 == 0))
			{
				damageThisTick += 1; // Accumulate +0.2 per tick -> exactly 3,600 total damage over 500 ticks!
			}

			Rect aGridRect(GridToPixelX(aGridItem->mGridX, aGridItem->mGridY), GridToPixelY(aGridItem->mGridX, aGridItem->mGridY), 80, 100);
			Zombie* aZombie = nullptr;
			while (IterateZombies(aZombie))
			{
				if (!aZombie->IsDeadOrDying() && aZombie->mRow == aGridItem->mGridY && aZombie->IsOnBoard())
				{
					Rect aZombieRect = aZombie->GetZombieRect();
					if (GetRectOverlap(aGridRect, aZombieRect) > 0)
					{
						aZombie->TakeDamage(damageThisTick, (1U << DAMAGE_BYPASSES_SHIELD));
					}
				}
			}
		}
...
void Board::DamageAllZombiesInRadius(int theRow, int theX, int theY, int theRadius, int theDamage, int theRowRange, bool theBurn, int theDamageRangeFlags)
{
	Zombie* aZombie = nullptr;
	while (IterateZombies(aZombie))
	{
		if (aZombie->EffectedByDamage((unsigned int)theDamageRangeFlags))
		{
			Rect aZombieRect = aZombie->GetZombieRect();
			int aRowDist = aZombie->mRow - theRow;
			if (aZombie->mZombieType == ZombieType::ZOMBIE_BOSS)
			...
			if (aRowDist <= theRowRange && aRowDist >= -theRowRange && GetCircleRectOverlap(theX, theY, theRadius, aZombieRect))
			{
				...
				{
					// Use the damage parameter instead of a fixed value
					aZombie->TakeDamage(theDamage, 1U << DAMAGE_BYPASSES_SHIELD);
				}
			}
		}
	}
...
void Board::UpgradePlant(int theGridX, int theGridY)
{
	Plant* aOldPlant = GetTopPlantAt(theGridX, theGridY, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
	if (aOldPlant == nullptr)
	{
		return; // No plant to upgrade
	}

	SeedType aNewSeedType = SeedType::SEED_NONE;

	// Check which plant is being upgraded and determine the new type
	switch (aOldPlant->mSeedType)
	{
	case SeedType::SEED_SUNFLOWER:
		aNewSeedType = SeedType::SEED_TWINSUNFLOWER;
		break;

	case SeedType::SEED_REPEATER:
		aNewSeedType = SeedType::SEED_GATLINGPEA;
		break;

	case SeedType::SEED_FUMESHROOM:
		aNewSeedType = SeedType::SEED_GLOOMSHROOM;
		break;

	case SeedType::SEED_LILYPAD:
		aNewSeedType = SeedType::SEED_CATTAIL;
		break;

	case SeedType::SEED_SPIKEWEED:
		aNewSeedType = SeedType::SEED_SPIKEROCK;
		break;

	case SeedType::SEED_MELONPULT:
		aNewSeedType = SeedType::SEED_WINTERMELON;
		break;

	case SeedType::SEED_MAGNETSHROOM:
		aNewSeedType = SeedType::SEED_GOLD_MAGNET;
		break;
	
	case SeedType::SEED_KERNELPULT:
	{
		// Find and remove the front Kernel-pult
		Plant* aFrontPlant = GetTopPlantAt(theGridX + 1, theGridY, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
		if (aFrontPlant != nullptr && aFrontPlant->mSeedType == SEED_KERNELPULT)
		{
			aFrontPlant->Die();
			aNewSeedType = SeedType::SEED_COBCANNON;
		}
		break;
	}

	case SeedType::SEED_PEASHOOTER:
		aNewSeedType = SeedType::SEED_FIRESHOOTER;
		break;

	case SeedType::SEED_WALLNUT:
		aNewSeedType = SeedType::SEED_EXPLODE_O_NUT;
		break;

	case SeedType::SEED_POTATOMINE:
		aNewSeedType = SeedType::SEED_RED_POTATO_MINE;
		break;

	case SeedType::SEED_SNOWPEA:
		aNewSeedType = SeedType::SEED_ICEPEA;
		break;

	case SeedType::SEED_CHOMPER:
		aNewSeedType = SeedType::SEED_SUPER_CHOMPER;
		break;

	case SeedType::SEED_HYPNOSHROOM:
		aNewSeedType = SeedType::SEED_HYPNOGIGA;
		break;

	case SeedType::SEED_SQUASH:
		aNewSeedType = SeedType::SEED_BOMBSQUASH;
		break;

	case SeedType::SEED_THREEPEATER:
		aNewSeedType = SeedType::SEED_BLACK_THREEPEATER;
		break;

	case SeedType::SEED_TORCHWOOD:
		aNewSeedType = SeedType::SEED_BLUE_TORCHWOOD;
		break;

	case SeedType::SEED_TALLNUT:
		aNewSeedType = SeedType::SEED_DOOM_NUT;
		break;

	case SeedType::SEED_PLANTERN:
		aNewSeedType = SeedType::SEED_ICE_PLANTERN;
		break;

	case SeedType::SEED_CACTUS:
		aNewSeedType = SeedType::SEED_MAD_CACTUS;
		break;

	case SeedType::SEED_STARFRUIT:
		aNewSeedType = SeedType::SEED_SOUR_STARFRUIT;
		break;

	case SeedType::SEED_CABBAGEPULT:
		aNewSeedType = SeedType::SEED_POISONPULT;
		break;

	case SeedType::SEED_GARLIC:
		aNewSeedType = SeedType::SEED_CORROSION_GARLIC;
		break;

	case SeedType::SEED_UMBRELLA:
		aNewSeedType = SeedType::SEED_ABSOLUTELEAF;
		break;

	case SeedType::SEED_SUNBEAN:
		aNewSeedType = SeedType::SEED_SUNBEAN_BOMB;
		break;

	case SeedType::SEED_COMMANDOPEA:
		aNewSeedType = SeedType::SEED_GENERALPEA;
		break;

	case SeedType::SEED_SWEETPOTATO:
		aNewSeedType = SeedType::SEED_SWEETEST_POTATO;
		break;

	case SeedType::SEED_HATTREMWITCH:
		aNewSeedType = SeedType::SEED_HATTREMSAGE;
		break;
	...
	if (aNewSeedType != SeedType::SEED_NONE)
	{
		...
		if (aNewPlant)
		{
			mApp->AddTodParticle(aNewPlant->mX + 40, aNewPlant->mY + 40, aNewPlant->mRenderOrder + 1, ParticleEffect::PARTICLE_IMITATER_MORPH);
		}
	}
}

TodParticleSystem* Board::AddAttachedParticle(GameObject* pObject, int thePosX, int thePosY, int theRenderOrder, ParticleEffect theEffect)
{
	if (!mApp)
		return nullptr;
	// Attach the particle system at the given position and render order
	TodParticleSystem* particle = mApp->AddTodParticle(static_cast<float>(thePosX), static_cast<float>(thePosY), theRenderOrder, theEffect);
	// Optionally, you can store the particle ID in the object if needed
	// Example: pObject->mParticleID = mApp->ParticleGetID(particle);
	return particle;
}

void Board::DamageRow(int theRow, int theDamage)
{
	Plant* aPlant = nullptr;
	while (IteratePlants(aPlant))
	{
		if (aPlant->mRow == theRow && !aPlant->NotOnGround())
		{
			aPlant->TakeDamage(theDamage);
		}
	}
}

void Board::PushPlantsInRow(int theRow)
{
	// Iterate through each column in the specified row
	for (int aCol = 0; aCol < MAX_GRID_SIZE_X; aCol++)
	{
		// Create a temporary list of plants to move from the current tile
		std::vector<Plant*> plantsOnTile;
		Plant* aPlant = nullptr;
		while (IteratePlants(aPlant))
		{
			if (aPlant->mRow == theRow && aPlant->mPlantCol == aCol)
			{
				plantsOnTile.push_back(aPlant);
			}
		}

		// Process the plants found on this tile
		for (Plant* p : plantsOnTile)
		{
			if (p->mPlantCol == 0) // If plant is in the first column
			{
				p->Die(); // It gets destroyed
			}
			else // If plant is in any other column
			{
				int aNewCol = p->mPlantCol - 1;
				// Check if the destination is empty
				if (GetTopPlantAt(aNewCol, theRow, TOPPLANT_ANY) == nullptr)
				{
					p->mPlantCol = aNewCol;
					p->mX = GridToPixelX(aNewCol, theRow);
					// Optional: Add a particle effect to show it was pushed
					mApp->AddTodParticle(p->mX + 40, p->mY + 40, p->mRenderOrder + 1, PARTICLE_DUST_SQUASH);
				}
				else
				{
					p->Die(); // Destination is blocked, so it gets destroyed
				}
			}
		}
	}
...
bool Board::IsNightRoof()
{
	return mBackground == BackgroundType::BACKGROUND_6_BOSS;
}

bool Board::IsEvening()
{
	return mBackground == BackgroundType::BACKGROUND_7_EVENING;
}

Zombie* Board::FindClosestZombie(int theX, int theY)
{
    Zombie* aBestZombie = nullptr;
    float aBestDistance = -1.0f;

    Zombie* aZombie = nullptr;
    while (IterateZombies(aZombie))
    {
        if (!aZombie->IsDeadOrDying() && !aZombie->IsFlying())
        {
            float aDist = Distance2D(theX, theY, aZombie->mX + 80, aZombie->mY + 60);
            if (aBestZombie == nullptr || aDist < aBestDistance)
            {
                aBestDistance = aDist;
                aBestZombie = aZombie;
            }
...
GridItem* Board::AddFirePlace(int theGridX, int theGridY, int theDamagePerTick)
{
	// Don't place fire on water tiles
	if (IsPoolSquare(theGridX, theGridY))
	{
		return nullptr;
	}

	GridItem* aFirePlace = mGridItems.DataArrayAlloc();
	aFirePlace->mGridItemType = GRIDITEM_FIRE_PLACE;
	aFirePlace->mGridItemCounter = 500; // 5 seconds (500 ticks)
	aFirePlace->mGridX = theGridX;
	aFirePlace->mGridY = theGridY;
	aFirePlace->mSunCount = theDamagePerTick;
	aFirePlace->mRenderOrder = MakeRenderOrder(RenderLayer::RENDER_LAYER_LAWN, theGridY, 0);

	// Add the fire animation to the grid item
	Reanimation* aFireReanim = mApp->AddReanimation(GridToPixelX(theGridX, theGridY) + 40, GridToPixelY(theGridX, theGridY), aFirePlace->mRenderOrder, REANIM_JALAPENO_FIRE);
	aFireReanim->mLoopType = REANIM_LOOP;
	aFireReanim->mAnimRate = RandRangeFloat(15.0f, 20.0f);
	aFireReanim->OverrideScale(0.8f, 0.9f);
	aFirePlace->mGridItemReanimID = mApp->ReanimationGetID(aFireReanim);
